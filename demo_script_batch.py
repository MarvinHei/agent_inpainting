...
dir = args.target_folder
folders = os.listdir(dir)
for folder in folders:
	print("Folder: ", folder)
	subfolders = os.listdir(os.path.join(dir, folder))
	rgb_arr = []
	masks_arr = []
	frames_lst = []
	for subfolder in subfolders:
		folder_path = os.path.join(dir, folder, subfolder)
		frames = os.listdir(os.path.join(folder_path, "raw"))
		frames.sort()
		rgb = sorted(glob(f"{folder_path}/raw/*.jpg",recursive=True))
		masks = sorted(glob(f"{folder_path}/masks/*.png",recursive=True))
		rgb = np.array(list(map(imread,rgb)))
		masks = np.array(list(map(imread,masks)))
		if len(masks.shape) == 3:
			reshaped_masks = np.zeros((masks.shape[0], masks.shape[1], masks.shape[2], 3))
			reshaped_masks[np.where(masks != 0)] = np.array([255, 255, 255])
			reshaped_masks[np.where(masks == 0)] = np.array([0, 0, 0])
		...
		rgb_arr.append(torch.tensor(batched_im))
		masks_arr.append(torch.tensor(batched_mask))
		frames_lst.extend(frames)
		#batched_im = np.array(rgb_arr)
		#batched_mask = np.array(masks_arr)
	batched_im = torch.cat(rgb_arr, 0)
	batched_mask = torch.cat(masks_arr, 0)
	print("BATCHED IM SHAPE!!!!!!!!!!! ", batched_im.shape)
	print("BATCHED MASK SHAPE!!!!!!!!!!!! ", batched_mask.shape)
	...
	if args.profiler:
		...
		if not os.path.exists("debug"):
			os.mkdir("debug")
		prof.export_chrome_trace("debug/trace.json")
	else:
		ddim_samples, z_denoise_row = model.sample_log(cond=cond, batch_size=cond.shape[0], ddim=True, ddim_steps=200, eta=1)
	# ddim_samples = model.sample(cond, batch_size=cond.shape[0])
	# ddim_samples = model(cond)
	...
	#counter = 0
	#for im in rgb:
	#	Image.fromarray(im).save(os.path.join(str(counter).zfill(7), ".jpg"))
	#	counter += 1
	...
	for i, im in enumerate(rgb):
		imwrite(os.path.join(out, frames_lst[i].split(".")[0] +  ".png"), im.astype(np.uint8)) 
