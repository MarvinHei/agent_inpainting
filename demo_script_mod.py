...
for folder in folders:
	...
	#indices = [[((batch_size - i) + (j * (batch_size-1))) % batch_size for i in range(batch_size)] for j in range(batch_size)]
        #indices = [[((batch_size - i) + (j* (batch_size-1))) % batch_size for i in range(batch_size)]]
	indices = [[((batch_size - i) + (j * (batch_size - 1))) % 4 + int(k*4) for i in range(4)] for k in range(int(batch_size/4)) for j in range(4)]
	...
	for i, im in enumerate(rgb):
		imwrite(os.path.join(out, frames[i].split(".")[0] +  ".png"), im.astype(np.uint8))
		counter += 1
